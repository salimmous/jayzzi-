...
-- Create new RLS policies with FOR ALL
CREATE POLICY "Users can manage their own articles v2"
    ON articles
    FOR ALL
    USING (auth.uid() = user_id)
    WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can manage pins from their articles v2"
    ON pins
    FOR ALL
    USING (article_id IN (
...
CREATE POLICY "Users can manage keywords v2"
    ON keywords
    FOR ALL
    TO authenticated
    USING (true)
